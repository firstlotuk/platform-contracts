...
const resultSchemaV11 = loadSchema('stateless-calculation-result/1.1.0/schema.json');
...
describe('D-132 stateless-calculation-result 1.1.0', () => {
  const validate = compile(resultSchemaV11);
  const validateV10 = compile(resultSchema);

  test('is a compilable draft 2020-12 schema', () => {
    expect(resultSchemaV11.$schema).toBe('https://json-schema.org/draft/2020-12/schema');
    expect(typeof validate).toBe('function');
  });
  ...
  test('required set is unchanged from 1.0.0', () => {
    ...
    expect(req(resultSchemaV11)).toEqual(req(resultSchema));
  });
});
