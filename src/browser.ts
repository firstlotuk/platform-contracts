// UNIVERSAL (client + edge + server safe) contract entry point.
//
// Runtime-domain boundary (FIR-579): this module — and everything it imports — must
// never import Ajv or any other heavy/server-only runtime dependency. It is legal to
...
// d067 S1 — person-core registry wire contract (pure types; Ajv-free by construction).
// auth.ts additions (svc-person-core principal, person-core audience) are owner-gated
// (plan D-011) and deliberately NOT made alongside this export — see src/person-core.ts banner.
export type {
  RegistryPersonId,
  ...
  AccountStatus,
  AccountOrigin,
  AccountMigrationProvenance,
  Account,
  AccountOwnership,
  AccountExternalRefKind,
  AccountExternalRef,
  ResolveMatchTier,
  ...
  RegistryEventFeedRequest,
  RegistryEventFeedResponse,
  CreateInstitutionRequest,
  UpdateInstitutionRequest,
  AccountExternalRefInput,
...
export {
  ...
  ACCOUNT_ORIGINS,
  isAccountOrigin,
  ACCOUNT_EXTERNAL_REF_KINDS,
  isAccountExternalRefKind,
  RESOLVE_MATCH_TIERS,
  isResolveMatchTier,
  RESOLVE_OUTCOMES,
